...
function applyTheme() {
  ...
  document.querySelector('#theme-color').setAttribute('content', active === 'dark' ? '#07111f' : '#f4f7fb');
...
function watchReason(item) {
  if (item.manual) return t('manualPin');
  const labels = {
    top_ranked: t('watchTopRanked'),
    github_trending_24h: t('watchTrending24h'),
    github_trending_7d: t('watchTrending7d'),
  };
  return labels[item.reason] || t('watchSelected');
}
...
function renderWatch() {
  ...
  document.querySelector('#watchlist').innerHTML = items.length ? items.map(x => `<article class="radar-row list-group-item d-flex justify-content-between gap-2" data-repo="${esc(x.full_name)}"><div class="min-w-0"><a href="${esc(github(x))}" target="_blank" rel="noreferrer noopener">${esc(x.full_name)} ↗</a><p class="text-muted-radar small mb-1">${esc(x.description || t('noDescription'))}</p><span class="badge text-bg-secondary rounded-pill mt-1">${watchReason(x)}</span></div><div class="text-end flex-shrink-0"><strong class="d-block">${fmt(x.stars)}</strong><small class="text-muted-radar d-block">${x.manual ? t('noExpiry') : `${t('until')} ${date(x.expires_at)}`}</small>${pinButton(x.full_name)}</div></article>`).join('') : `<p class="text-muted-radar p-3 mb-0">${t('watchEmpty')}</p>`;
  bindInteractiveCards('#watchlist');
}

function selectWatchSort(sort) {
  watchSort = sort;
  storageSet('radar-watch-sort', sort);
  renderWatch();
}
...
document.querySelectorAll('.language-button').forEach(button => button.onclick = () => { language = button.dataset.language; storageSet('radar-language', language); applyLanguage(); applyTheme(); load(); });
document.querySelector('#theme-toggle').onclick = () => { theme = ({ system: 'light', light: 'dark', dark: 'system' })[theme]; storageSet('radar-theme', theme); applyTheme(); };
